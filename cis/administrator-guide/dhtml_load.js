...
function bodyLoad() {
    ...
    var allElements = document.getElementsByClassName("relatedtopics aboveheading")[0].getElementsByTagName("a");
    if (allElements !== null) {
        for (counter = 0; counter < allElements.length; counter++) {
            allElements[counter].removeAttribute("target");
        }
    }

    //Removes content button from Nav Bar(relatedtopics aboveheading) because it is no longer needed
    allElements = document.getElementsByClassName("relatedtopics aboveheading")[0].getElementsByTagName("td");
    if (allElements !== null) {
        for (counter = 0; counter < allElements.length; counter++) {
            var element = allElements[counter];
    ...
    if (location.pathname.substring(location.pathname.length - 4) === '.htm') {
        var targetId = location.pathname.substr(-10, 6);
